...
def test_create_review_draft() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/v1.0/me/messages"
        assert request.headers["Authorization"] == "Bearer test-token"
        payload = request.read().decode()
        assert "Outlook integration" in payload
...
def test_missing_access_token(monkeypatch) -> None:
    monkeypatch.delenv("MICROSOFT_GRAPH_ACCESS_TOKEN", raising=False)

    with pytest.raises(OutlookServiceError) as exc_info:
        OutlookService(recipient="reviewer@example.com")

    assert exc_info.value.status_code == 503
    assert exc_info.value.detail == "Microsoft Graph access token is not configured"
...
def test_missing_recipient(monkeypatch) -> None:
    ...
    with pytest.raises(OutlookServiceError) as exc_info:
        OutlookService(access_token="test-token")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (403, "Microsoft Graph Mail.ReadWrite permission is required"),
